...
static void print_vecf(tph_poisson_vec *v)
{
  printf("mem = %" PRIxPTR " | mem_size = %" PRIu32 " | size = %" PRIu32 "\n",
    (uintptr_t)v->mem,
    (uint32_t)v->mem_size,
    (uint32_t)(tph_poisson_vec_size(v, sizeof(float))));
  printf("v = { ");
...
int main(int argc, char *argv[])
{
  ...
  tph_poisson_vec_append(&v, &tph_poisson_default_alloc, a, 6, sizeof(float), alignof(float));
  tph_poisson_vec_append(&v, &tph_poisson_default_alloc, b, 6, sizeof(float), alignof(float));
  print_vecf(&v);

  tph_poisson_vec_erase_unordered(&v, 3, 2, sizeof(float));
  print_vecf(&v);

  tph_poisson_vec_erase_unordered(&v, 7, 2, sizeof(float));
  print_vecf(&v);
